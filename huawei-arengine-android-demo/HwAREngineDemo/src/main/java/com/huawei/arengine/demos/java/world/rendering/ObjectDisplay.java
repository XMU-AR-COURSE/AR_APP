...
public class ObjectDisplay {
     ...
     */
    public void init(Context context,int obj_now) {  //传入模型名参数obj_now,该方法为VirtualObject的初始化
        ...
        initializeGlObjectData(context,obj_now);//为初始化模型传入obj_now参数
    ...
    private void initializeGlObjectData(Context context,int obj_now) { //obj_now为需要初始化模型的名称
        ObjectData objectData = null;
        Optional<ObjectData> objectDataOptional = readObject(context,obj_now);//读取相应的obj，传入路径
    ...
    private Optional<ObjectData> readObject(Context context,int obj_now) { //obj_now为当前模型的名称
        Obj obj;
        try (InputStream objInputStream = context.getAssets().open("objs/"+obj_now+".obj")) {  //替换路径
